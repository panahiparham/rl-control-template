...
_N1 = jax.nn.initializers.normal(stddev=1.0)


@chex_struct(frozen=True, kw_only=True)
class BadInitDQNConfig:
    LR: float = 3e-4
    ...
    SEED: int = 42
    NETWORK_PRESET: Literal["mlp", "nature_cnn", "ln-mlp", "ln-mlp-no-affine"] = "mlp"
...
class QNetwork(TypedApply[jax.Array], nn.Module):
    ...
    def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
        x = nn.Dense(64, kernel_init=_N1, bias_init=_N1)(x)
        x = nn.relu(x)
        x = nn.Dense(64, kernel_init=_N1, bias_init=_N1)(x)
        x = nn.relu(x)
        x = nn.Dense(self.action_dim, kernel_init=_N1, bias_init=_N1)(x)
        return x
...
class _HasNetworkPreset(Protocol):
    @property
    def NETWORK_PRESET(self) -> Literal["mlp", "nature_cnn", "ln-mlp", "ln-mlp-no-affine"]: ...
...
class NatureQNetwork(TypedApply[jax.Array], nn.Module):
    ...
    def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
        ...
        x = NatureCNN()(x)
        x = nn.Dense(512, kernel_init=_N1, bias_init=_N1)(x)
        x = nn.relu(x)
        x = nn.Dense(self.action_dim, kernel_init=_N1, bias_init=_N1)(x)
        return x
...
def _make_q_network(
...
) -> QNetwork | LNQNetwork | LNNoAffineQNetwork | NatureQNetwork:
    ...
    raise ValueError(
        f"Invalid NETWORK_PRESET {config.NETWORK_PRESET!r}. Expected one of: 'mlp', 'ln-mlp', 'ln-mlp-no-affine', 'nature_cnn'."
    )
...
class BadInitDQNTrainOutput(TypedDict):
    runner_state: RunnerState
    metrics: dict[str, jax.Array]


def make_train(config: BadInitDQNConfig, env: GymEnv[DiscreteActionSpace], env_params: object | None = None) -> Callable[[jax.Array, dict | None], BadInitDQNTrainOutput]:
    def train(rng: jax.Array, hypers: dict | None = None) -> BadInitDQNTrainOutput:
        if hypers is None:
            hypers = {}
